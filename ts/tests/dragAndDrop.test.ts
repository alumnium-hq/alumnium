...
describe('Drag and Drop Tests', () => {
  ...
  it('drag and drop', async function() {
    ...
    const initialOrder = await al.get('titles of squares ordered from left to right', true);
    assert.deepStrictEqual(initialOrder, ['A', 'B']);

    await al.do('move square A to square B');

    const finalOrder = await al.get('titles of squares ordered from left to right', true);
