...
class TestToolDefinitions:
    ...
    def test_tool_list_includes_new_tools(self):
        """Test that new tools are included in the tool list."""
        from alumnium.mcp.tools import get_tool_definitions
    ...
    def test_wait_for_element_schema(self):
        """Test that wait_for_element has correct schema."""
        from alumnium.mcp.tools import get_tool_definitions

        tools = get_tool_definitions()
        wait_for_element = next(t for t in tools if t.name == "wait_for_element")

        schema = wait_for_element.inputSchema
        assert "driver_id" in schema["properties"]
        assert "selector" in schema["properties"]
        assert "timeout" in schema["properties"]
        assert "driver_id" in schema["required"]
        assert "selector" in schema["required"]
        # timeout should be optional with default
