...
class SeleniumDriver(BaseDriver):
    ...
    def _capture_full_page_screenshot(self) -> str:
        """Capture full page screenshot using scroll-and-stitch approach.

        This method scrolls through the page and captures viewport-sized screenshots,
        then stitches them together. This produces better results than CDP's
        captureBeyondViewport for pages with complex scrolling, sticky elements,
        or lazy-loaded content.
        """
        # Disable sticky/fixed elements to prevent them from appearing in every screenshot
        self._disable_sticky_elements()

        try:
            return self._stitch_screenshots()
        finally:
            # Re-enable sticky/fixed elements
            self._enable_sticky_elements()

    def _disable_sticky_elements(self):
        """Temporarily disable position:fixed and position:sticky elements."""
        self.driver.execute_script("""
            // Find all fixed/sticky elements and store their original position
            const elements = document.querySelectorAll('*');
            window.__alumniumStickyElements = [];

            for (const el of elements) {
                const style = getComputedStyle(el);
                if (style.position === 'fixed' || style.position === 'sticky') {
                    window.__alumniumStickyElements.push({
                        element: el,
    ...
    def _enable_sticky_elements(self):
        """Restore original position for fixed/sticky elements."""
        self.driver.execute_script("""
            if (window.__alumniumStickyElements) {
                for (const item of window.__alumniumStickyElements) {
                    item.element.style.position = item.originalPosition;
                    item.element.style.top = item.originalTop;
                }
                delete window.__alumniumStickyElements;
            }
        """)

    def _stitch_screenshots(self) -> str:
        """Capture and stitch viewport screenshots into a full page image."""
        # Get page dimensions
        ...
        total_width = dimensions["totalWidth"]
        total_height = dimensions["totalHeight"]
        original_scroll_x = dimensions["originalScrollX"]
        original_scroll_y = dimensions["originalScrollY"]
        device_pixel_ratio = dimensions["devicePixelRatio"]

        # Calculate actual pixel dimensions (accounting for device pixel ratio)
        actual_viewport_width = int(viewport_width * device_pixel_ratio)
        actual_viewport_height = int(viewport_height * device_pixel_ratio)
        actual_total_width = int(total_width * device_pixel_ratio)
        actual_total_height = int(total_height * device_pixel_ratio)

        logger.debug(
            f"Full page screenshot: viewport={viewport_width}x{viewport_height}, "
            f"total={total_width}x{total_height}, dpr={device_pixel_ratio}"
        )
        ...
        final_image = Image.new("RGB", (actual_total_width, actual_total_height))
        ...
        for i in range(num_screenshots):
            ...
            # For the last screenshot, adjust to capture the bottom of the page
            is_last = i == num_screenshots - 1
            if is_last and total_height > viewport_height:
            ...
            self.driver.execute_script("return new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));")

            # Capture screenshot
            screenshot_data = self.driver.get_screenshot_as_base64()
            screenshot_image = Image.open(io.BytesIO(base64.b64decode(screenshot_data)))

            # Calculate where to paste this screenshot
            paste_y = int(scroll_y * device_pixel_ratio)

            # For the last screenshot, we need to handle overlap
            if is_last and i > 0:
                # Calculate how much of this screenshot to use (from the bottom)
                remaining_height = actual_total_height - paste_y
                if remaining_height < actual_viewport_height:
                    # Crop the top portion that would overlap with previous screenshot
                    crop_top = actual_viewport_height - remaining_height
                    screenshot_image = screenshot_image.crop(
                        (0, crop_top, screenshot_image.width, screenshot_image.height)
                    )

            final_image.paste(screenshot_image, (0, paste_y))
            logger.debug(f"  -> Screenshot {i + 1}/{num_screenshots} at scroll_y={scroll_y}, paste_y={paste_y}")

        # Restore original scroll position
        self.driver.execute_script(f"window.scrollTo({original_scroll_x}, {original_scroll_y});")
