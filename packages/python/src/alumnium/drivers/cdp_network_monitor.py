...
IGNORED_URL_SCHEMES = ("blob:", "data:", "about:", "javascript:")
...
class CdpNetworkMonitor:
    ...
    def _request_started(self, params: dict, session_id: str):
        ...
        with self._lock:
            if params.get("type") in IGNORED_RESOURCE_TYPES or url.startswith(IGNORED_URL_SCHEMES):
                self._pending.pop(key, None)
                return
            self._pending[key] = PendingRequest(url, monotonic())
            self._last_activity_at = monotonic()
