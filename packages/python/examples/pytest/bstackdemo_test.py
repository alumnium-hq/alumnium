...
def test_checkout(al):
    ...
    assert al.get("iPhone 12 Pro Max price (without money sign)").data == 1099
    assert al.get("iPhone 12 Mini price (without money sign)").data == 699
    assert al.get("total amount (without money sign)").data == 1798
