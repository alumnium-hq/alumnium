...
def test_table_extraction(al, navigate):
    ...
    assert area.get("Jason Doe's due amount").data == "$100.00"
    assert area.get("Frank Bach's due amount").data == "$51.00"
    assert area.get("Tim Conway's due amount").data == "$50.00"
    assert area.get("John Smith's due amount").data == "$50.00"
...
def test_table_sorting(al, navigate):
    ...
    assert table1.get("first names").data == ["John", "Frank", "Jason", "Tim"]
    assert table1.get("last names").data == ["Smith", "Bach", "Doe", "Conway"]

    table2 = al.area("example 2 table - return table element")
    assert table2.get("first names").data == ["John", "Frank", "Jason", "Tim"]
    assert table2.get("last names").data == ["Smith", "Bach", "Doe", "Conway"]

    table1.do("sort by last name")
    table1 = al.area("example 1 table - return table element")  # refresh
    assert table1.get("first names").data == ["Frank", "Tim", "Jason", "John"]
    assert table1.get("last names").data == ["Bach", "Conway", "Doe", "Smith"]
    ...
    table2 = al.area("example 2 table - return table element")  # refresh
    assert table2.get("first names").data == ["Frank", "Jason", "John", "Tim"]
    assert table2.get("last names").data == ["Bach", "Doe", "Smith", "Conway"]
...
def test_retrieval_of_unavailable_data(al, navigate):
    ...
    assert al.get("atomic number of Selenium").data is None
