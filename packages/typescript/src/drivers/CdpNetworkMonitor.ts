...
const IGNORED_URL_SCHEMES = ["blob:", "data:", "about:", "javascript:"];
...
export class CdpNetworkMonitor {
  ...
  process(method: string, params: object, sessionId = ""): void {
    if (method === "Network.requestWillBeSent") {
      ...
      if (
        (event.type && IGNORED_RESOURCE_TYPES.has(event.type)) ||
        IGNORED_URL_SCHEMES.some((scheme) => url.startsWith(scheme))
      ) {
        this.#pending.delete(key);
        return;
      }
      this.#pending.set(key, { url, received: 0 });
