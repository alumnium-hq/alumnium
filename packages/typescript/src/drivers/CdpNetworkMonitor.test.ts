...
describe(CdpNetworkMonitor, () => {
  ...
  test("ignores in-memory URLs that Chrome never reports as finished", () => {
    const monitor = new CdpNetworkMonitor();
    monitor.process("Network.requestWillBeSent", {
      requestId: "worker",
      type: "Other",
      request: { url: "blob:https://example.com/3bb82b96-7d78-4de9" },
    });
    monitor.process("Network.requestWillBeSent", {
      requestId: "inline",
      type: "Image",
