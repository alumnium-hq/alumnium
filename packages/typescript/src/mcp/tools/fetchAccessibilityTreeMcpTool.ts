...
export const fetchAccessibilityTreeMcpTool = McpTool.define({
  ...
  description: txt`
    Get structured representation of current page for debugging. Useful for
    understanding page structure or debugging.
  `,
  ...
  async execute(input) {
    ...
    return tree.toXml(client.session.excludeAttributes);
  },
});
