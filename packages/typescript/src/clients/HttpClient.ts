...
export class HttpClient extends Client {
  ...
  async getModel(): Promise<Model> {
    return tracer.span("client.get_model", this.#spanAttrs(), async () => {
  ...
  async getHealth(): Promise<HealthCheckResponse> {
    return tracer.span("client.get_health", this.#spanAttrs(), () =>
      this.#fetch<HealthCheckResponse>("GET", "/health"),
    );
  }
  ...
  async planActions(
    ...
    return tracer.span("client.plan_actions", this.#spanAttrs(), () => {
  ...
  async executeAction(
    ...
    return tracer.span("client.execute_action", this.#spanAttrs(), () => {
  ...
  async findArea(
    ...
    return tracer.span("client.find_area", this.#spanAttrs(), async () => {
      ...
      const data = await this.#sessionFetch<AreaResponse>(
        "POST",
        "/areas",
        body,
      );
  ...
  async saveCache(): Promise<void> {
    return tracer.span("client.save_cache", this.#spanAttrs(), async () => {
      await this.#sessionFetch("POST", "/caches");
    });
  }

  async discardCache(): Promise<void> {
    return tracer.span("client.discard_cache", this.#spanAttrs(), async () => {
      await this.#sessionFetch("DELETE", "/caches");
    });
  }

  async getStats(): Promise<LlmUsageStats> {
    return tracer.span("client.get_stats", this.#spanAttrs(), () =>
      this.#sessionFetch<LlmUsageStats>("GET", "/stats"),
    );
  }
