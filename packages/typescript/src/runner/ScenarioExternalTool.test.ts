...
describe("ScenarioExternalTool", () => {
  // NOTE: External servers are only spawned by a call to one of their tools, so
  // the tests below that never make one leave this unconnected.
  const mcp = new ScenarioExternalMcp();

  const execute = (name: string, input: Record<string, unknown>) =>
    ScenarioExternalTool.execute(name, input, mcp);
  ...
  describe("isSupported", () => {
    ...
    it("supports the tools of a configured MCP server", () => {
      const server = Object.keys(ScenarioExternalMcp.mcpServers())[0];

      expect(ScenarioExternalTool.isSupported(`mcp__${server}__anyTool`)).toBe(
        true,
      );
      expect(ScenarioExternalTool.isSupported("mcp__nope__anyTool")).toBe(
        false,
      );
    });
  });
  ...
  describe("Bash", () => {
    it("returns command output", async () => {
      await expect(
        execute("Bash", { command: "echo 4; echo 8" }),
    ...
    it("fails when the input has no command", async () => {
      await expect(execute("Bash", {})).resolves.toMatchObject({
        status: "failure",
      });
    });
  });
  ...
  describe("Read", () => {
    ...
    it("returns file contents", async () => {
      ...
      await expect(execute("Read", { file_path: filePath })).resolves.toEqual({
        status: "executed",
        output: "one\ntwo\nthree\nfour\n",
      });
    });
  ...
  describe("unknown tools", () => {
    it("reports them as unsupported with a reason", async () => {
      await expect(execute("ToolSearch", { query: "x" })).resolves.toEqual({
        status: "unsupported",
        reason: "no executor for 'ToolSearch'",
      });
    });

    it("names the MCP server that is not configured", async () => {
      await expect(execute("mcp__nope__createGuest", {})).resolves.toEqual({
        status: "unsupported",
        reason: "no MCP server configured for 'mcp__nope__createGuest'",
      });
    });
  });
});
