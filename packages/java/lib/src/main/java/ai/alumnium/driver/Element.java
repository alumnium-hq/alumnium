...
/**
 * Sealed wrapper over the three native element types returned by the supported drivers.
 *
 ...
 * }</pre>
 *
 * <p>Each record's canonical accessor returns the concrete native type via covariant return, so
 * {@code element.raw()} satisfies the interface declaration without explicit overrides.
 */
