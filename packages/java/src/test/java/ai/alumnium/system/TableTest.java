...
public class TableTest extends BaseTest {
  ...
  void testTableExtraction() {
    ...
    Assertions.assertEquals(
        List.of("John", "Frank", "Jason", "Tim"), table1.get("first names"));
    Assertions.assertEquals(
        List.of("Smith", "Bach", "Doe", "Conway"), table1.get("last names"));
    Assertions.assertEquals(
        List.of("$50.00", "$51.00", "$100.00", "$50.00"), table1.get("due amounts"));

    Area table2 = al.area("second table");
    Assertions.assertEquals(
        List.of("John", "Frank", "Jason", "Tim"), table2.get("first names"));
    Assertions.assertEquals(
        List.of("Smith", "Bach", "Doe", "Conway"), table2.get("last names"));
    Assertions.assertEquals(
  ...
  void testTableSorting() {
    ...
    Assertions.assertEquals(
        List.of("Frank", "Tim", "Jason", "John"), table1.get("first names"));
    Assertions.assertEquals(
        List.of("Bach", "Conway", "Doe", "Smith"), table1.get("last names"));
    ...
    Assertions.assertEquals(
        List.of("Frank", "Jason", "John", "Tim"), table2.get("first names"));
    Assertions.assertEquals(
        List.of("Bach", "Doe", "Smith", "Conway"), table2.get("last names"));
