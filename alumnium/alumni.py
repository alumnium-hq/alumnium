...
class Alumni:
    def __init__(self, driver: Page | WebDriver, model: Model = Model.load()):
        ...
        if model == Model.AZURE_OPENAI:
            ...
            llm = ChatAnthropic(model=model.value, temperature=0, top_p=1)
            ...
            llm = ChatDeepSeek(model=model.value, temperature=0, top_p=1)
        elif model == Model.GOOGLE:
            llm = ChatGoogleGenerativeAI(model=model.value, temperature=0, top_p=1)
        elif model == Model.OPENAI:
            llm = ChatOpenAI(model=model.value, temperature=0, top_p=1, seed=1)
        else:
