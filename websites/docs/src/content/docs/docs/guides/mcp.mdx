...
```bash
codex mcp add alumnium --env ALUMNIUM_MODEL=codex -- uvx alumnium mcp
```
...
```bash
codex mcp add alumnium --env ALUMNIUM_MODEL=codex -- npx alumnium mcp
```
...
:::tip[Reuse your Cursor subscription]
You can point Alumnium at [Cursor Agents][12] instead of OpenAI by swapping the `env` block. See the [Cursor configuration][13] for details.

```json
"env": {
  "ALUMNIUM_MODEL": "cursor",
  "CURSOR_API_KEY": "..."
}
```
